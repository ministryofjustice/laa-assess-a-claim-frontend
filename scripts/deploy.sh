...
# Set the deployment host, this will add the prefix of the branch name e.g el-257-deploy-with-circleci or just main
  RELEASE_HOST="$BRANCH_RELEASE_NAME-laa-assess-a-claim-frontend-uat.cloud-platform.service.justice.gov.uk"
# Set the ingress name, needs release name, namespace and -green suffix
  IDENTIFIER="$BRANCH_RELEASE_NAME-laa-assess-a-claim-frontend-$K8S_NAMESPACE-green"
  echo "Deploying commit: $GITHUB_SHA under release name: '$BRANCH_RELEASE_NAME'..."

  helm upgrade "$BRANCH_RELEASE_NAME" ./deploy/laa-assess-a-claim-frontend/. \
                --install --wait \
                --namespace="${K8S_NAMESPACE}" \
                --values ./deploy/laa-assess-a-claim-frontend/values/"$ENVIRONMENT".yaml \
...
deploy_main() {  
  helm upgrade laa-assess-a-claim-frontend ./deploy/laa-assess-a-claim-frontend/. \
                          --install --wait \
                          --namespace="${K8S_NAMESPACE}" \
                          --values ./deploy/laa-assess-a-claim-frontend/values/"$ENVIRONMENT".yaml \
                          --set image.repository="$REGISTRY/$REPOSITORY" \
