...
const copyAssets = async () => {
    try {
        ...
        await fs.copy(path.resolve('./node_modules/govuk-frontend/dist/govuk/assets/rebrand'), path.resolve('./public/assets/rebrand'));
        // Copy MOJ Frontend assets
        await fs.copy(path.resolve('./node_modules/@ministryofjustice/frontend/moj/assets/images'), path.resolve('./public/assets/images'));
 ...
 */
const buildScss = async (watch = false) => {
    const options = {
 ...
 * @returns {Promise<esbuild.BuildContext | undefined>} Build context if watching, undefined otherwise
 */
const buildAppJs = async (watch = false) => {
    const options = {
 ...
 * @returns {Promise<esbuild.BuildContext | undefined>} Build context if watching, undefined otherwise
 */
const buildCustomJs = async (watch = false) => {
    const options = {
 ...
 * @returns {Promise<esbuild.BuildContext | undefined>} Build context if watching, undefined otherwise
 */
const buildFrontendPackages = async (watch = false) => {
    const options = {
        entryPoints: [
...
const watchBuild = async () => {
    try {
        ...
        const handleSigint = () => {
            console.log('\n🛑 Stopping watch mode...');
            void Promise.all(contexts
                .filter((context) => context !== undefined)
                .map(async (context) => { await context.dispose(); })).then(() => {
