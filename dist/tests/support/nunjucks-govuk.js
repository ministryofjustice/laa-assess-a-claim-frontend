...
function pkgRoot(pkg) {
    ...
    for (;;) {
        ...
        if (up === dir)
            throw new Error(`Cannot locate root for ${pkg}`);
        dir = up;
    }
}
...
export function setupNunjucksForGovUk() {
    ...
    const env = new nunjucks.Environment(new nunjucks.FileSystemLoader([appViews, govuk, moj], { noCache: true }), { autoescape: true });
    // Minimal global your base.njk needed
    env.addGlobal('getAsset', (p) => `/assets/${p}`);
    return env;
}
//# sourceMappingURL=nunjucks-govuk.js.map