...
describe("views/main/error.njk", () => {
  ...
  it("renders the status in the main H1", () => {
    const h1 = $(
      ".govuk-grid-row .govuk-grid-column-two-thirds h1.govuk-heading-l",
    )
      .text()
      .trim();
  ...
  it("renders the error message paragraph", () => {
    const p = $(".govuk-grid-row .govuk-grid-column-two-thirds p.govuk-body")
      .text()
      .trim();
  ...
  it("applies the large main wrapper class (from mainClasses)", () => {
    ...
    expect(hasClass, "expected .govuk-main-wrapper--l to be present").to.be
      .true;
  });
});
