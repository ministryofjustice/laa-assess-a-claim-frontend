...
describe("ClaimViewModel constructor()", () => {
  ...
  it("builds the summary rows", () => {
    ...
    expect(vm.summaryRows[0].key.text.key).to.equal("pages.claim.summary.totalClaimAmount");
    expect(vm.summaryRows[0].value).to.deep.equal({ type: "text", value: "£3,480" });
    expect(vm.summaryRows[0].actions).to.be.undefined;

    expect(vm.summaryRows[1].key.text.key).to.equal("pages.claim.summary.dateReceived");
    ...
    expect(vm.summaryRows[2].key.text.key).to.equal("pages.claim.summary.caseReferenceNumber");
    expect(vm.summaryRows[2].value).to.deep.equal({type: "text", value: "300001820960"});
    expect(vm.summaryRows[2].actions).to.be.undefined;

    expect(vm.summaryRows[3].key.text.key).to.equal("pages.claim.summary.laaReferenceNumber");
    expect(vm.summaryRows[3].value).to.deep.equal({type: "text", value: "LAA-90d26c"});
    expect(vm.summaryRows[3].actions).to.be.undefined;

    expect(vm.summaryRows[4].key.text.key).to.equal("pages.claim.summary.assignedTo");
    ...
    expect(vm.summaryRows[5].key.text.key).to.equal("pages.claim.summary.providerRisk");
    ...
    expect(vm.summaryRows[6].key.text.key).to.equal("pages.claim.summary.claimTimeStandard");
  ...
  it("builds the costs and allocations rows when escaped", () => {
    ...
    expect(rows[0].key.text.key).to.equal("pages.claim.costsAndAllocations.claimType");
    expect(rows[0].value).to.deep.equal({type: "text", value: "Solicitor final bill"});
    expect(rows[0].actions).to.be.undefined;

    expect(rows[1].key.text.key).to.equal("pages.claim.costsAndAllocations.totalClaimAmount");
    ...
    expect(rows[2].key.text.key).to.equal("pages.claim.costsAndAllocations.fixedFeeAmountGranted");
    expect(rows[2].value).to.deep.equal({type: "text", value: "£3,000"});
    expect(rows[2].actions).to.be.undefined;

    expect(rows[3].key.text.key).to.equal("pages.claim.costsAndAllocations.escapeThreshold");
    expect(rows[3].value).to.deep.equal({type: "text", value: "£6,000"});
    expect(rows[3].actions).to.be.undefined;

    expect(rows[4].key.text.key).to.equal("pages.claim.costsAndAllocations.assessmentBasis");
    expect(rows[4].value).to.deep.equal({type: "text", value: "Hourly rate, escaped"});
    expect(rows[4].actions).to.be.undefined;

    expect(rows[5].key.text.key).to.equal("pages.claim.costsAndAllocations.counselCostAndAllocation");
    expect(rows[5].value).to.deep.equal({type: "text", value: "£2,850"});
    expect(rows[5].actions).to.be.undefined;

    expect(rows[6].key.text.key).to.equal("pages.claim.costsAndAllocations.totalPaymentOnAccount");
    expect(rows[6].value).to.deep.equal({type: "text", value: "£1,200"});
    expect(rows[6].actions).to.be.undefined;

    expect(rows[7].key.text.key).to.equal("pages.claim.costsAndAllocations.totalPOA");
    expect(rows[7].value).to.deep.equal({type: "text", value: "£1,200"});
    expect(rows[7].actions).to.be.undefined;

    expect(rows[8].key.text.key).to.equal("pages.claim.costsAndAllocations.priorAuthority");
    ...
    expect(rows[9].key.text.key).to.equal("pages.claim.costsAndAllocations.availableCostLimit");
  ...
  it("builds the costs and allocations rows when fixed", () => {
    ...
    expect(rows[2].key.text.key).to.equal("pages.claim.costsAndAllocations.assessmentBasis");
    expect(rows[2].value).to.deep.equal({type: "text", value: "Fixed fee applies"});
    expect(rows[2].actions).to.be.undefined;

    expect(rows[3].key.text.key).to.equal("pages.claim.costsAndAllocations.counselCostAndAllocation");
    expect(rows[3].value).to.deep.equal({type: "text", value: "£900"});
    expect(rows[3].actions).to.be.undefined;

    expect(rows[4].key.text.key).to.equal("pages.claim.costsAndAllocations.availableCostLimit");
  ...
  it("builds the provider rows when counsel payment is defined", () => {
    ...
    expect(rows[0].key.text.key).to.equal("pages.claim.providers.solicitorName");
    expect(rows[0].value).to.deep.equal({type: "text", value: "Smith & Co Solicitors"});
    expect(rows[0].actions).to.be.undefined;

    expect(rows[1].key.text.key).to.equal("pages.claim.providers.solicitorRegion");
    expect(rows[1].value).to.deep.equal({type: "text", value: "North West"});
    expect(rows[1].actions).to.be.undefined;

    expect(rows[2].key.text.key).to.equal("pages.claim.providers.numberOfSolicitors");
    expect(rows[2].value).to.deep.equal({type: "text", value: "1"});
    expect(rows[2].actions).to.be.undefined;

    expect(rows[3].key.text.key).to.equal("pages.claim.providers.counselInvolved");
    expect(rows[3].value).to.deep.equal({type: "text", value: { key: "common.yes" }});
    expect(rows[3].actions).to.be.undefined;

    expect(rows[4].key.text.key).to.equal("pages.claim.providers.counselPayment");
  ...
  it("builds the client rows", () => {
    ...
    expect(rows[0].key.text.key).to.equal("pages.claim.client.name");
    expect(rows[0].value).to.deep.equal({type: "text", value: "Liam Oldfield"});
    expect(rows[0].actions).to.be.undefined;

    expect(rows[1].key.text.key).to.equal("pages.claim.client.dateOfBirth");
    expect(rows[1].value).to.deep.equal({type: "text", value: "27 March 1996"});
    expect(rows[1].actions).to.be.undefined;

    expect(rows[2].key.text.key).to.equal("pages.claim.client.location");
    expect(rows[2].value).to.deep.equal({type: "text", value: "Manchester"});
    expect(rows[2].actions).to.be.undefined;

    expect(rows[3].key.text.key).to.equal("pages.claim.client.status");
  ...
  it("builds the case rows", () => {
    ...
    expect(rows[0].key.text.key).to.equal("pages.case.summary.matterType");
    expect(rows[0].value).to.deep.equal({type: "text", value: "Special Children Act"});
    expect(rows[0].actions).to.be.undefined;

    expect(rows[1].key.text.key).to.equal("pages.case.summary.leadProceeding");
    expect(rows[1].value).to.deep.equal({type: "text", value: "Care order"});
    expect(rows[1].actions).to.be.undefined;

    expect(rows[2].key.text.key).to.equal("pages.case.summary.linkedCases");
    expect(rows[2].value).to.deep.equal({ type: "link", value: { text: "1 linked case", href: "#" }});
    expect(rows[2].actions).to.be.undefined;

    expect(rows[3].key.text.key).to.equal("pages.case.summary.outcome");
  ...
  it("builds the proceedings rows", () => {
    ...
    expect(rows[0].key.text.key).to.equal("pages.case.proceedings.careOrder");
    ...
    expect(rows[1].key.text.key).to.equal("pages.case.proceedings.supervisionOrder");
